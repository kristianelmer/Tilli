...
create table if not exists public.billing_payment_events (
  ...
  kind text not null check (kind in ('subscription', 'subscription_cancellation', 'filing_package', 'refund')),
  status text not null check (status in ('created', 'succeeded', 'failed', 'refunded', 'canceled')),
